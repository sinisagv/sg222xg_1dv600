...
public class Game {
	...
	public void playTimedGame() throws IOException {
		Highscore highscore = new Highscore();
		highscore.setGame(this);
		int numOfMistakes = 0;
		String word = "";
		...
		System.out.println("The word has now been randomly selected. To quit the game, press any non letter key.");
		System.out.println(
				"This is a timed game mode, you gave 60 seconds to guess the correct word, may the Force be with you.");
		guessedLetters = new char[word.length()];
		for (int i = 0; i < guessedLetters.length; i++) {
			guessedLetters[i] = ' ';
		}
		...
		do {
			...
			if (!Character.isLetter(guessLetter)) {
				...
				if (Long.compare((timer - startTime) / 1000000000, maxTime) >= 0) {
					break;
				}
				guessedLetters = printLettersAndLines(word, guessLetter);
				isGameWon = checkVictory(word);
				...
				&& Long.compare((timer - startTime) / 1000000000, maxTime) <= 0);
		if (isGameWon) {
			System.out.println();
			if (player != null) {
				System.out.println("Your score: " + (60 - ((timer - startTime) / 1000000000)) * 300);
				highscore.writeHighScore();
				player.setCurrentScore(0);
			}
	...
	private void playAgainTimed() throws IOException {
	...
	public void mainMenu() throws IOException {
		...
		System.out.println("7. Play timed game mode");
		Scanner sc = new Scanner(System.in);
		String choice = "";
		choice = sc.next();
		switch (choice) {
